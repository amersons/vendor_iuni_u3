...
static VOS_STATUS wlan_ftm_priv_set_power_index(hdd_adapter_t *pAdapter,ePowerTempIndexSource pwr_source)
{
    uPttMsgs *pMsgBody;
    VOS_STATUS status;
    hdd_context_t *pHddCtx = (hdd_context_t *)pAdapter->pHddCtx;
    ...
    if (pwr_source > 3)
    {
        VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL, "%s:invalid power index source. valid mode is 0 , 1, 2. ",__func__);
        return VOS_STATUS_E_FAILURE;
    }

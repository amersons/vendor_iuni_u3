...
)
{
  ...
  switch(usb_uicc_global.bridge_type)
  {
    case USB_UICC_SPI_BRIDGE_SOLN_TYPE:
      LOGI("Starting enumeration for SPI Bridge");
      property_set(USB_UICC_SPI_BRIDGE_ENABLED, "1");
      break;

    case USB_UICC_HSIC_HUB_SOLN_TYPE:
      LOGI("Starting enumeration for HSIC HUB");
      ret = system("echo 1 > /sys/bus/platform/devices/msm_smsc_hub/enable");
      break;

    case USB_UICC_DIRECT_CONNECT_SOLN_TYPE:
      LOGI("Starting enumeration for EHCI");
      ret = system("echo msm_ehci_host > /sys/bus/platform/drivers/msm_ehci_host/bind");
      break;

    default:
      LOGE("invalid bridge type = %d\n", usb_uicc_global.bridge_type);
  }
...
)
{
  ...
  {
    LOGE("ccid bridge has been already closed!");
  }
  ...
  switch(usb_uicc_global.bridge_type)
  {
    ...
    case USB_UICC_DIRECT_CONNECT_SOLN_TYPE:
      LOGI("Terminate enumeration for EHCI\n");
      ret = system("echo msm_ehci_host > /sys/bus/platform/drivers/msm_ehci_host/unbind");
      break;

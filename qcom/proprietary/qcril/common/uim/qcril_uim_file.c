...
--------   ---     ----------------------------------------------------------
07/07/14   yt      Return 6D 00 for invalid instruction in APDU
  ...
  if (original_request_ptr->request_id == RIL_REQUEST_SIM_OPEN_CHANNEL)
  {
    ...
    else
    {
          ...
          original_request_ptr->data.channel_info.fci_value == QCRIL_UIM_FCI_VALUE_FCI_FALLBACK_FCP)
      {
        ...
        logical_channel_params.channel_data.aid.data_ptr =
          original_request_ptr->data.channel_info.aid_buffer;
        logical_channel_params.channel_data.aid.data_len =
          original_request_ptr->data.channel_info.aid_size;
        logical_channel_params.slot =
          original_request_ptr->data.channel_info.slot;
        logical_channel_params.file_control_information.fci_value =
          QMI_UIM_FCI_VALUE_FCP;
...
)
{
  ...
  if (params_ptr->event_id == RIL_REQUEST_SIM_OPEN_CHANNEL)
  {
    ...
    const char * in_ptr = params_ptr->data;
    ...
    {
      QCRIL_LOG_ERROR("%s", "Error converting AID string: 0x%x", aid_size);
      goto logical_channel_error;
    }
    ...
    {
      qcril_fci_value =
        qcril_uim_determine_select_template_from_aid(in_ptr);
  ...
  if (params_ptr->event_id == RIL_REQUEST_SIM_OPEN_CHANNEL)
  {
    ...
    original_request_ptr->data.channel_info.slot = logical_channel_params.slot;
  }
...
)
{
  ...
  if ((request_io_ptr->command & 0xF0) == QCRIL_UIM_INVALID_INS_BYTE_MASK)
  {
    memset(&ril_response, 0, sizeof(RIL_SIM_IO_Response));

    QCRIL_LOG_ERROR( "Invalid INS byte 0x%X", request_io_ptr->command);

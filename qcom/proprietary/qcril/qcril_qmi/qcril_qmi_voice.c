  ...
  else
  {
    ...
    do
    {
        ...
        if (subaddr)
        {
           ...
           if (subaddr_len > 0)
           {
              const uint SUBADDR_LEN_MAX = QMI_VOICE_SUBADDRESS_LEN_MAX_V02 - 2; // Need to reserve one byte for padding 0x50 and one byte for QMI VOICE to add the NULL terminator

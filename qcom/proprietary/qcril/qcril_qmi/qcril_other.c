...
/* Gionee liuxw 2013-07-16 add for factory NV begin */
#if defined (CONFIG_GN_Q_BSP_FACTORY_NV_SUPPORT)
#include <sys/stat.h>
#include "qmi_i.h"
#include "qmi_util.h"
#endif
/* Gionee liuxw 2013-07-16 add for factory NV end */
/*===========================================================================
...
/* Gionee liuxw 2013-07-16 add for factory NV begin */
  ...
  ,GN_NV_EGMR_I                                  = 9000
  ,GN_NV_GHDVER_I                                = 9001
  ,GN_NV_APSW_VERSION_INFO_I                     = 9002
  ,GN_NV_GPS_NUM_SV_IN_VIEW_I                    = 9003
  ,GN_NV_FACTORY_SET_I                           = 9004
  ,GN_NV_WIFIDLS_INFO_I                          = 9005
  ,GN_NV_BPSW_VERSION_INFO_I                     = 9006
  /* Gionee liuxw 2014-06-26 add for sglte open market begin */
  ,GN_NV_OPEN_MARKET_FLAG_I                     = 9007
  /* Gionee liuxw 2014-06-26 add for sglte open market end */
#endif
/* Gionee liuxw 2013-07-16 add for factory NV end */
} nv_items_enum_type;
...
#define QMI_OTHER_STD_MSG_SIZE                                QMI_MAX_STD_MSG_SIZE
#define QMI_OTHER_MAX_PARAM_BUF_SIZE                          (255)

/* Request Response Message IDs */
#define QMI_OTHER_SET_APSW_VERSION_INFO_REQ_V01         (0x5556)
#define QMI_OTHER_SET_APSW_VERSION_INFO_RESP_V01        (0x5556)
#define QMI_OTHER_GET_WLAN_ADDR_REQ_V01                 (0x5557)
#define QMI_OTHER_GET_WLAN_ADDR_RESP_V01                (0x5557)
#define QMI_OTHER_GET_FACTORY_REQ_V01                   (0x5558)
#define QMI_OTHER_SET_FACTORY_REQ_V01                   (0x5559)
#define QMI_OTHER_GET_EGMR_REQ_V01                      (0x555A)
/* Gionee liuxw 2014-05-07 add for factory nv begin */
#define QMI_OTHER_GET_WIFIDLS_INFO_REQ_V01                   (0x555B)
#define QMI_OTHER_SET_WIFIDLS_INFO_REQ_V01                   (0x555C)
/* Gionee liuxw 2014-05-07 add for factory nv end */
/* Gionee liuxw 2014-06-26 add for sglte open market begin */
#define QMI_OTHER_GET_OPEN_MARKET_FLAG_REQ_V01          (0x555D)
#define QMI_OTHER_SET_OPEN_MARKET_FLAG_REQ_V01          (0x555E)
/* Gionee liuxw 2014-06-26 add for sglte open market end */

#define NV_MAX_SW_VERSION_INFO_SIZ                      (30)

/* Request TLV IDs */
#define QMI_OTHER_MANDATORY_1_TLV_ID                    (0x01)

#define  NV_GN_EGMR_MAX_SIZE                               (32 + 1)
#define  NV_GN_GHDVER_MAX_SIZE                             (64 + 1)
#define  NV_GN_APSW_MAX_SIZE                               (92 + 1)
#define  NV_GN_WLAN_ADDR_MAX_SIZE                          (6)
#define  NV_GN_FACTORY_TEST_FLAG_MAX_SIZE                  (32 + 1)

#define  NV_GN_PROP_VALUE_MAX                             (92)
#define  NV_GN_FAC_VERSION_HEAD                           "FAC:"
#define  NV_GN_FAC_VERSION_HEAD_SIZE                      (4)
#define  NV_GN_FAC_VERSION_MAX_SIZE                       (35)
#define  NV_GN_AP_VERSION_HEAD                            "AP:"
#define  NV_GN_AP_VERSION_HEAD_SIZE                       (3)
#define  NV_GN_AP_VERSION_MAX_SIZE                        (57)
/* define ap sw version */
#define QCRIL_OTHER_FAC_SW_VERSION      "ro.gn.gnvernumber"
#define QCRIL_OTHER_AP_SW_VERSION       "ro.gn.gnznvernumber"

/* Gionee liuxw 2013-8-17 add for write egmr to system property begin */
#define QCRIL_OTHER_SYS_PROP_SERIAL     "persist.radio.serial"
/* Gionee liuxw 2013-8-17 add for write egmr to system property end */

/* define max tries for factory nv init  */
#define MAX_TRIES_FOR_FACTORY_NV_INIT    (2)

extern struct qmi_client_struct {
...
#if defined (CONFIG_GN_Q_BSP_FACTORY_NV_SUPPORT)
typedef PACKED struct PACKED_POST{ 
  /* Test WifiDLS info flag */
  byte                                              flag;
  /*  Rate of  the Wifi */
  uint32                                           Wifidls;
}nv_Wifidls_info_type;
#endif
/* Gionee liuxw 2013-07-16 add for factory nv end*/
...
/* Gionee liuxw 2013-07-16 add for factory nv begin */
#if defined (CONFIG_GN_Q_BSP_FACTORY_NV_SUPPORT)
  byte                                             egmr_info[NV_GN_EGMR_MAX_SIZE];
  byte                                             ghdver_info[NV_GN_GHDVER_MAX_SIZE];
  byte                                             apsw_version_info[NV_GN_APSW_MAX_SIZE];
  byte                                             num_sv_in_view;
  byte                                             factory_set[NV_GN_FACTORY_TEST_FLAG_MAX_SIZE];
  nv_Wifidls_info_type                             WifiDLS_info;
  byte                                             bpsw_version_info[NV_MAX_SW_VERSION_INFO_SIZ];
  /* Gionee liuxw 2014-06-26 add for sglte open market begin */
  byte                                             open_market_flag;
  /* Gionee liuxw 2014-06-26 add for sglte open market end */
#endif
/* Gionee liuxw 2013-07-16 add for factory nv end */
...
/* Gionee liuxw 2013-07-16 add for factory NV begin */
#if defined (CONFIG_GN_Q_BSP_FACTORY_NV_SUPPORT)
  { GN_NV_WIFIDLS_INFO_I,       QCRIL_OTHER_NV_SO( WifiDLS_info ),        "NV_WIFIDLS_INFO_I" },
  { GN_NV_EGMR_I,               QCRIL_OTHER_NV_SO( egmr_info ),           "GN_NV_EGMR_I" },
  { GN_NV_GHDVER_I,             QCRIL_OTHER_NV_SO( ghdver_info ),         "GN_NV_GHDVER_I" },
  { GN_NV_APSW_VERSION_INFO_I,  QCRIL_OTHER_NV_SO( apsw_version_info ),   "GN_NV_APSW_VERSION_INFO_I" },
  { GN_NV_GPS_NUM_SV_IN_VIEW_I, QCRIL_OTHER_NV_SO( num_sv_in_view ),      "GN_NV_GPS_NUM_SV_IN_VIEW_I" },
  { GN_NV_FACTORY_SET_I,        QCRIL_OTHER_NV_SO( factory_set ),         "GN_NV_FACTORY_SET_I" },
  { GN_NV_BPSW_VERSION_INFO_I,  QCRIL_OTHER_NV_SO( bpsw_version_info ),   "GN_NV_BPSW_VERSION_INFO_I" },
  /* Gionee liuxw 2014-06-26 add for sglte open market begin */
  { GN_NV_OPEN_MARKET_FLAG_I,  QCRIL_OTHER_NV_SO( open_market_flag ),   "GN_NV_OPEN_MARKET_FLAG_I" },
  /* Gionee liuxw 2014-06-26 add for sglte open market end */
#endif
/* Gionee liuxw 2013-07-16 add for factory NV end */
};
...
/* Gionee liuxw 2013-07-16 add for factory nv begin */
#if defined (CONFIG_GN_Q_BSP_FACTORY_NV_SUPPORT)
void qcril_other_set_apsw_version(void);
/* Gionee liuxw 2013-8-17 add for write egmr to system property begin */
void  qcril_other_get_egmr_version(void);
/* Gionee liuxw 2013-8-17 add for write egmr to system property end */
#endif
/* Gionee liuxw 2013-07-16 add for factory nv end */
/*===========================================================================
...
/* Gionee liuxw 2013-07-16 add for factory nv begin */
  ...
  {
    qcril_other_set_apsw_version();
	/* Gionee liuxw 2013-8-17 add for write egmr to system property begin */
    qcril_other_get_egmr_version();
	/* Gionee liuxw 2013-8-17 add for write egmr to system property end */
  }
#endif
/* Gionee liuxw 2013-07-16 add for factory nv end */
...
/* Gionee liuxw 2013-07-16 add for factory NV begin */
#if defined (CONFIG_GN_Q_BSP_FACTORY_NV_SUPPORT)
      case GN_NV_FACTORY_SET_I:
         {
            byte              factory_set[NV_GN_FACTORY_TEST_FLAG_MAX_SIZE] = {0};
            unsigned char     msg[QMI_OTHER_STD_MSG_SIZE];
            ...
            boolean           get_factory_result = FALSE;
		
            /*Prepare the Final TLV*/
            tmp_msg_ptr = QMI_SRVC_PDU_PTR(msg);
            msg_size = QMI_SRVC_PDU_SIZE(QMI_OTHER_STD_MSG_SIZE);
			
            if (E_SUCCESS !=
                qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_GET_FACTORY_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  0,
                                  msg,
                                  &msg_size,
                                  QMI_OTHER_STD_MSG_SIZE,
                                  QMI_SYNC_MSG_DEFAULT_TIMEOUT,
                                  &qmi_err_code))
            {
               QCRIL_LOG_INFO("%s", "qcril_qmi_client_send_msg_sync returned error");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                                RIL_E_GENERIC_FAILURE, NULL, 0);
               break;
            }
            ...
            while (msg_size > 0)
            {
               if (qmi_util_read_std_tlv (&tmp_msg_ptr,
                                    ...
                                    &value_ptr) < 0)
               {
                   QCRIL_LOG_ERROR ("%s", " read std tlv error. \n");
                   break;
               }

               switch (type)
               {
                   case QMI_OTHER_MANDATORY_1_TLV_ID:
                   {
                       if((int)length > NV_GN_FACTORY_TEST_FLAG_MAX_SIZE)
                       {
                           QCRIL_LOG_ERROR("%s","length of the response greater than the resp buff size. \n ");
                           break; 
                       }
                       memcpy(factory_set,value_ptr,NV_GN_FACTORY_TEST_FLAG_MAX_SIZE);
                       get_factory_result = TRUE;
                   }		  
                   break;

                   default:
                   {
                       QCRIL_LOG_INFO("unknown response TLV type = %x", (unsigned int)type);
                   }
                   break;
               }
            }
            ...
            /* check response status */
            if( !get_factory_result)
            {
               QCRIL_LOG_INFO("%s", "Response is invalid");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                                RIL_E_GENERIC_FAILURE, NULL, 0);
               break;
            }

               memcpy((void *)nv_item.factory_set ,
               (void *)factory_set,
               NV_GN_FACTORY_TEST_FLAG_MAX_SIZE);	

               QCRIL_LOG_INFO("nv_item.factory_set val: %d", nv_item.factory_set);
      ...
      case GN_NV_EGMR_I:
         {
            byte              egmr_info[NV_GN_EGMR_MAX_SIZE] = {0};
            unsigned char     msg[QMI_OTHER_STD_MSG_SIZE];
            unsigned char     *tmp_msg_ptr;
            int               msg_size =0;
            int               qmi_err_code =0;
            ...
            while (msg_size > 0)
            {
               ...
               switch (type)
               {
                   case QMI_OTHER_MANDATORY_1_TLV_ID:
                   {
                       if((int)length > NV_GN_EGMR_MAX_SIZE)
                       {
                           QCRIL_LOG_ERROR("%s","length of the response greater than the resp buff size. \n ");
                           break; 
                       }
                       memcpy(egmr_info,value_ptr,NV_GN_EGMR_MAX_SIZE);
                       get_egmr_result = TRUE;
                   }		  
                   break;
            ...
            if( !get_egmr_result)
            {
               ...
               memcpy((void *)nv_item.egmr_info ,
               (void *)egmr_info,
               NV_GN_EGMR_MAX_SIZE);	

               QCRIL_LOG_INFO("nv_item.egmr_info val: %d", nv_item.egmr_info);
      ...
      case GN_NV_WIFIDLS_INFO_I:
         {
            ...
            unsigned long     length;
            unsigned char     *value_ptr;
            boolean           get_wifidls_info = FALSE;
	     nv_Wifidls_info_type wifidls_info;
		
            memset(&wifidls_info, 0, sizeof(wifidls_info));
            ...
            if (E_SUCCESS !=
                qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_GET_WIFIDLS_INFO_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  0,
                                  msg,
                                  &msg_size,
            ...
            while (msg_size > 0)
            {
               ...
               switch (type)
               {
                   case QMI_OTHER_MANDATORY_1_TLV_ID:
                   {
                       if((int)length > sizeof(wifidls_info))
                       {
                           QCRIL_LOG_ERROR("%s","length of the response greater than the resp buff size. \n ");
                           break; 
                       }
                       READ_8_BIT_VAL (value_ptr, nv_item.WifiDLS_info.flag);
                       READ_32_BIT_VAL (value_ptr, nv_item.WifiDLS_info.Wifidls);
                       get_wifidls_info = TRUE;
                   }		  
                   break;
            ...
            QCRIL_LOG_INFO("nv_item.WifiDLS_info val: flag %d, wifids %d", nv_item.WifiDLS_info.flag, nv_item.WifiDLS_info.Wifidls);
            qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                             RIL_E_SUCCESS, (void *) &nv_item, qcril_other_nv_table[index].nv_item_size);
         }
         break;
      ...
      case GN_NV_OPEN_MARKET_FLAG_I:
         {
            ...
            unsigned long     length;
            unsigned char     *value_ptr;
            boolean           get_open_market_flag = FALSE;
            ...
            if (E_SUCCESS !=
                qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_GET_OPEN_MARKET_FLAG_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  0,
                                  msg,
                                  &msg_size,
            ...
            while (msg_size > 0)
            {
               ...
               switch (type)
               {
                   case QMI_OTHER_MANDATORY_1_TLV_ID:
                   {
                       if((int)length > sizeof(byte))
                       {
                           QCRIL_LOG_ERROR("%s","length of the response greater than the resp buff size. \n ");
                           break; 
                       }
                       READ_8_BIT_VAL (value_ptr, nv_item.open_market_flag);
                       get_open_market_flag = TRUE;
                   }		  
                   break;
            ...
            QCRIL_LOG_INFO("nv_item.open_market_flag  %d", nv_item.open_market_flag);
            qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                             RIL_E_SUCCESS, (void *) &nv_item, qcril_other_nv_table[index].nv_item_size);
         }
         break;
...
/* Gionee liuxw 2013-07-16 add for factory NV begin */
      ...
      case GN_NV_FACTORY_SET_I:
         {
            unsigned char     msg[QMI_OTHER_STD_MSG_SIZE];
            unsigned char     param_buf[QMI_OTHER_MAX_PARAM_BUF_SIZE];
            ...
            memcpy( tmp_param_ptr,
                        (void *)nv_item.factory_set,
                        NV_GN_FACTORY_TEST_FLAG_MAX_SIZE);	
            tmp_param_ptr +=NV_GN_FACTORY_TEST_FLAG_MAX_SIZE;
		 
            if (qmi_util_write_std_tlv (&tmp_msg_ptr,
                                    &msg_size,
                                    QMI_OTHER_MANDATORY_1_TLV_ID,
                                    sizeof(nv_item.factory_set),
                                    (void *)param_buf) < 0 )
            {
               QCRIL_LOG_INFO("%s", "qmi_util_write_std_tlv returned error");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                                RIL_E_GENERIC_FAILURE, NULL, 0);
               break;
            }
 
            rc = qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle ( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_SET_FACTORY_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  (int)QMI_SRVC_PDU_SIZE(QMI_OTHER_STD_MSG_SIZE) - msg_size,
                                  msg,
                                  &msg_size,
                                  QMI_OTHER_STD_MSG_SIZE,
                                  QMI_SYNC_MSG_DEFAULT_TIMEOUT,
            ...
            /* check response status */
            if (  QMI_NO_ERR ==  rc &&  QMI_SERVICE_ERR_NONE == qmi_err_code)      
            {
               QCRIL_LOG_INFO("%s", "Set factory result  success");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                             RIL_E_SUCCESS, NULL, 0);
            }
	        else
            {
               QCRIL_LOG_INFO("%s", "Set factory result error ");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                                RIL_E_GENERIC_FAILURE, NULL, 0);
            }
         }
      ...
      case GN_NV_WIFIDLS_INFO_I:
         {
            ...
            /* Construct Terminal response TLV (mandatory) */
            tmp_param_ptr = param_buf;
            WRITE_8_BIT_VAL (tmp_param_ptr, nv_item.WifiDLS_info.flag);
            WRITE_32_BIT_VAL (tmp_param_ptr, nv_item.WifiDLS_info.Wifidls);
            tmp_param_ptr += SIZE_8_BIT_VAL + SIZE_32_BIT_VAL;
		 
            if (qmi_util_write_std_tlv (&tmp_msg_ptr,
                                    &msg_size,
                                    QMI_OTHER_MANDATORY_1_TLV_ID,
                                    (SIZE_8_BIT_VAL + SIZE_32_BIT_VAL),
                                    (void *)param_buf) < 0 )
            {
               QCRIL_LOG_INFO("%s", "qmi_util_write_std_tlv returned error");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
            ...
            rc = qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle ( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_SET_WIFIDLS_INFO_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  (int)QMI_SRVC_PDU_SIZE(QMI_OTHER_STD_MSG_SIZE) - msg_size,
                                  msg,
                                  &msg_size,
      ...
      case GN_NV_OPEN_MARKET_FLAG_I:
         {
            ...
            /* Construct Terminal response TLV (mandatory) */
            tmp_param_ptr = param_buf;
            WRITE_8_BIT_VAL (tmp_param_ptr, nv_item.open_market_flag);
            tmp_param_ptr += SIZE_8_BIT_VAL;
            ...
            rc = qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle ( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_SET_OPEN_MARKET_FLAG_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  (int)QMI_SRVC_PDU_SIZE(QMI_OTHER_STD_MSG_SIZE) - msg_size,
                                  msg,
                                  &msg_size,
            ...
            if (  QMI_NO_ERR ==  rc &&  QMI_SERVICE_ERR_NONE == qmi_err_code)      
            {
               QCRIL_LOG_INFO("%s", "Set open_market_flag  success");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                             RIL_E_SUCCESS, NULL, 0);
            }
	        else
            {
               QCRIL_LOG_INFO("%s", "Set open_market_flag error ");
               qcril_other_send_request_reponse(instance_id, params_ptr->t, params_ptr->event_id,
                                                RIL_E_GENERIC_FAILURE, NULL, 0);
            }
         }
...
/*===========================================================================
  ...
  FUNCTION:  qcril_other_set_apsw_version

===========================================================================*/
/*!
    @brief
    Set apsw version in NV.
...
void  qcril_other_set_apsw_version(void)
{
  char              apsw_version_info[NV_GN_APSW_MAX_SIZE];
  unsigned char     msg[QMI_OTHER_STD_MSG_SIZE];
  unsigned char     param_buf[QMI_OTHER_MAX_PARAM_BUF_SIZE];
  unsigned char     * tmp_msg_ptr   = NULL;
  unsigned char     * tmp_param_ptr = NULL;
  ...
  char              gn_facsw_version_info[NV_GN_PROP_VALUE_MAX];
  char              gn_apsw_version_info[NV_GN_PROP_VALUE_MAX];
  char              gn_relsw_version_info[NV_GN_PROP_VALUE_MAX];
  char         	    *get_fac_space = NULL; 

  /*-----------------------------------------------------------------------*/
  QCRIL_LOG_INFO("%s", "qcril_other_set_apsw_version entered" );

  /* initialize the property value buffer */
  (void)memset(gn_facsw_version_info, 0x00, sizeof(gn_facsw_version_info) );
  if(property_get( QCRIL_OTHER_FAC_SW_VERSION,gn_facsw_version_info, "" ) > 0)
  {
    QCRIL_LOG_INFO( "Set fac sw version = %s\n", gn_facsw_version_info);
  }
  else
  {
    QCRIL_LOG_ERROR("%s", "Failed to get fac sw version from system property\n");  
    return;
  }

  (void)memset(gn_apsw_version_info, 0x00, sizeof(gn_apsw_version_info) );
  if ( property_get( QCRIL_OTHER_AP_SW_VERSION,gn_apsw_version_info, "" ) > 0) 
  {
    QCRIL_LOG_INFO( "Set ap sw version = %s\n", gn_apsw_version_info);
  }
  else
  {
    QCRIL_LOG_ERROR("%s", "Failed to get ap sw version from system property\n");
    return;
  }

  (void)memset(apsw_version_info, 0x20, sizeof(apsw_version_info));
  (void)memcpy(apsw_version_info, NV_GN_FAC_VERSION_HEAD, NV_GN_FAC_VERSION_HEAD_SIZE);
  (void)memcpy(&apsw_version_info[NV_GN_FAC_VERSION_HEAD_SIZE], gn_facsw_version_info,
  	                  MIN( strlen(gn_facsw_version_info), NV_GN_FAC_VERSION_MAX_SIZE - NV_GN_FAC_VERSION_HEAD_SIZE ));

  apsw_version_info[strlen(apsw_version_info)] = 0x20;
  (void)memcpy(&apsw_version_info[NV_GN_FAC_VERSION_MAX_SIZE], NV_GN_AP_VERSION_HEAD, NV_GN_AP_VERSION_HEAD_SIZE);
  (void)memcpy(&apsw_version_info[NV_GN_FAC_VERSION_MAX_SIZE + NV_GN_AP_VERSION_HEAD_SIZE],gn_apsw_version_info,
  	                  MIN(strlen(gn_apsw_version_info),NV_GN_AP_VERSION_MAX_SIZE - NV_GN_AP_VERSION_HEAD_SIZE));
  apsw_version_info[NV_GN_APSW_MAX_SIZE - 1] = '\0';
  ...
  /* Construct Terminal response TLV (mandatory) */
  tmp_param_ptr = param_buf;
  memcpy(tmp_param_ptr, (void *)apsw_version_info, NV_GN_APSW_MAX_SIZE);
  tmp_param_ptr +=NV_GN_APSW_MAX_SIZE;
  ...
  if (qmi_util_write_std_tlv (&tmp_msg_ptr,
                    ...
                    (void *)param_buf) < 0 )
  {
    QCRIL_LOG_ERROR("%s", "Failed to write std tlv\n"); 
    return;
  }
  ...
  do
  {
    ...
    rc = qmi_service_send_msg_sync (qcril_qmi_client_get_user_handle ( QCRIL_QMI_CLIENT_DMS )->service_user_handle,
                                  QMI_DMS_SERVICE,
                                  QMI_OTHER_SET_APSW_VERSION_INFO_REQ_V01,
                                  QMI_SRVC_PDU_PTR(msg),
                                  (int)QMI_SRVC_PDU_SIZE(QMI_OTHER_STD_MSG_SIZE) - msg_size,
                                  msg,
                                  &msg_size,
    ...
    if ( rc < QMI_NO_ERR )                              
    {
      QCRIL_LOG_ERROR("Set ap sw version: rc = %d, error code = %d, num tries = %d", rc, qmi_err_code, num_tries); 
    }
	
    num_tries += 1;
	
  }while ( rc != QMI_NO_ERR && num_tries < MAX_TRIES_FOR_FACTORY_NV_INIT);
  
  QCRIL_LOG_INFO("%s", "qcril_other_set_apsw_version completed" );
  return; 
}
...
/*!
    @brief
    Get egmr from modem and set it in system property.
...
void  qcril_other_get_egmr_version(void)
{
  char			   egmr_info[NV_GN_EGMR_MAX_SIZE] = {0};
  char			   serial_info[NV_GN_EGMR_MAX_SIZE] = {0};
  unsigned char	   msg[QMI_OTHER_STD_MSG_SIZE];
  unsigned char	   *tmp_msg_ptr;
  int			   msg_size =0;
  int			   qmi_err_code =0;
  ...
  /*-----------------------------------------------------------------------*/
  QCRIL_LOG_INFO("%s", "qcril_other_get_egmr_version entered" );
  ...
  }while ( rc != QMI_NO_ERR && get_egmr_result != TRUE && num_tries < MAX_TRIES_FOR_FACTORY_NV_INIT);

  /* check result */
  if( get_egmr_result && egmr_info[0]!= '\0')
  {
	 /* get serial property. */
	 ret = property_get(QCRIL_OTHER_SYS_PROP_SERIAL, serial_info, "");
	 if (ret <= NV_GN_EGMR_MAX_SIZE && strcmp(egmr_info, serial_info))
	 {	    
        if ( E_SUCCESS != property_set(QCRIL_OTHER_SYS_PROP_SERIAL, egmr_info ) )
        {
           QCRIL_LOG_INFO("%s", "Fail to save egmr to system property\n");
        }
	 }
	 else
	 {
        QCRIL_LOG_INFO("property_get for serial returned %d size", ret);
	 }
  }
    
  QCRIL_LOG_INFO("%s", "qcril_other_get_egmr_version completed" );
  return; 
}
/* Gionee liuxw 2013-8-17 add for write egmr to system property end */

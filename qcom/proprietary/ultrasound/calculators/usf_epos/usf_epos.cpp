 ...
 */
static void close_and_sync_file(FILE *file)
{
  if (NULL != file)
  {
...
int epos_exit (int status)
{
  ...
  close_and_sync_file(s_work_params.frameFile);
  s_work_params.frameFile = NULL;

  close_and_sync_file(s_work_params.coordFile);
...
void epos_init_smarter_stand()
{
  ...
  long screen_distance = fabs(on_screen_origin.get_element(Z) -
  ...
  { // It's time to check spurs
      ...
      if (!ret)
      {
        ...
        if (isSpur != s_work_params.is_spur)
        {
          ...
          LOGD("%s: ret=%d; is_spur=%d; spur_freq=%ld",
               __FUNCTION__,
               ret,

...
public class QcNvItemTypes {
    ...
    public static class NvAutoAnswerType {
     ...
     /* Gionee liuxw 2013-07-13 add for gn_correlative_nv begin */	 
     //EGMR NV9000
     public static class NvEgmrType extends BaseQCNvItemType {

        byte mNvByteArray[] = new byte[IQcNvItems.NV_MAX_BP_EGMR_SIZE];

        public NvEgmrType() {
            mNvByteArray = new byte[IQcNvItems.NV_MAX_BP_EGMR_SIZE];
        }

        NvEgmrType(byte[] bArray) throws IOException {
            ByteBuffer buf = createByteBuffer(bArray);
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_EGMR_SIZE; i++)
                this.mNvByteArray[i] = buf.get();
        }

        public void setNvEgmrNumber(String in) throws InvalidParameterException {

            if (in.length() >= IQcNvItems.NV_MAX_BP_EGMR_SIZE)
                throw new InvalidParameterException();

            for (int i = 0; i < in.length(); i++) {
		        Character c = in.charAt(i);
		        mNvByteArray[i] = (byte)in.charAt(i);
		    }
            for(int i = in.length(); i < IQcNvItems.NV_MAX_BP_EGMR_SIZE ; i++){	
		        mNvByteArray[i] = 0;
            }	
        }

        public String getNvEgmrNumber() {
            String ret = "";
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_EGMR_SIZE -1; i++) {
				if(mNvByteArray[i] == 0) {
				    break;
				} else {
                    ret += (char)mNvByteArray[i];
				}
            }
            return ret;
        }
        ...
        public byte[] toByteArray() {
            ByteBuffer buf = createByteBuffer(getSize());
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_EGMR_SIZE; i++)
                buf.put(this.mNvByteArray[i]);
            return buf.array();
        }

        public int getSize() {
            return (SIZE_OF_BYTE * IQcNvItems.NV_MAX_BP_EGMR_SIZE );
        }

        public String toDebugString() {
            return String.format("dir_number:%s", getNvEgmrNumber());
        }
     };
	 
     //GHDVER NV9001
     public static class NvGhdverType extends BaseQCNvItemType {

        byte mNvByteArray[] = new byte[IQcNvItems.NV_MAX_BP_GHDVER_SIZE];

        public NvGhdverType() {
            mNvByteArray = new byte[IQcNvItems.NV_MAX_BP_GHDVER_SIZE];
        }

        NvGhdverType(byte[] bArray) throws IOException {
            ByteBuffer buf = createByteBuffer(bArray);
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_GHDVER_SIZE; i++)
                this.mNvByteArray[i] = buf.get();
        }

        public void setNvGhdverNumber(String in) throws InvalidParameterException {

            if (in.length() >= IQcNvItems.NV_MAX_BP_GHDVER_SIZE)
                throw new InvalidParameterException();
            ...
            for(int i = in.length(); i < IQcNvItems.NV_MAX_BP_GHDVER_SIZE -1; i++){	
		        mNvByteArray[i] = 0;
            }		
        }

        public String getNvGhdverNumber() {
            String ret = "";
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_GHDVER_SIZE -1; i++) {
				if(mNvByteArray[i] == 0) {
				    break;
				} else {
                    ret += (char)mNvByteArray[i];
        ...
        public int getSize() {
            return (SIZE_OF_BYTE * IQcNvItems.NV_MAX_BP_GHDVER_SIZE );
        }

        public String toDebugString() {
            return String.format("dir_number:%s", getNvGhdverNumber());
        }
     };

     //factory set NV9004
     public static class NvFactoryType extends BaseQCNvItemType {

        byte mNvByteArray[] = new byte[IQcNvItems.NV_MAX_BP_FACTORY_SIZE];

        public NvFactoryType() {
            mNvByteArray = new byte[IQcNvItems.NV_MAX_BP_FACTORY_SIZE];
        }

        NvFactoryType(byte[] bArray) throws IOException {
            ByteBuffer buf = createByteBuffer(bArray);
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_FACTORY_SIZE; i++)
                this.mNvByteArray[i] = buf.get();
        }

        public void setNvFactoryNumber(String in) throws InvalidParameterException {

            if (in.length() != IQcNvItems.NV_MAX_BP_FACTORY_SIZE)
                throw new InvalidParameterException();
        ...
        public String getNvFactoryNumber() {
            String ret = "";
            for (int i = 0; i < IQcNvItems.NV_MAX_BP_FACTORY_SIZE -1; i++) {
				if(mNvByteArray[i] == 0){
				    ret +=  (char)'0';
				} else { 
                    ret += (char)mNvByteArray[i];
        ...
        public int getSize() {
            return (SIZE_OF_BYTE * IQcNvItems.NV_MAX_BP_FACTORY_SIZE );
        }

        public String toDebugString() {
            return String.format("dir_number:%s", getNvFactoryNumber());
        }
    };

    // wifidls_info NV9005
    public static class NvWifiDLSType extends BaseQCNvItemType{
        public byte mFlag; 

        public int mDls; 

        NvWifiDLSType( ) throws InvalidParameterException {
	        this.mFlag= 0;
	        this.mDls = 0;	            
        }

        NvWifiDLSType(byte mFlag,int mDls) throws InvalidParameterException {
            setFlag(mFlag);
            setDls(mDls);
        }
	
        NvWifiDLSType(byte[] bArray) throws IOException {
            ByteBuffer buf = createByteBuffer(bArray);
            this.mFlag= buf.get();
            skipPaddingBytes(buf, 3);		
            this.mDls= buf.getInt();
        }
        ...
        public void setFlag(byte mFlag)throws InvalidParameterException{
		    this.mFlag = mFlag;
        }
        ...
        public void setDls(int mDls)throws InvalidParameterException{
            this.mDls= mDls;
        }
        ...
        public byte[] toByteArray() {
            ...
            buf.put(this.mFlag);
            addPaddingBytes(buf, 3);
            buf.putInt(this.mDls);
            return buf.array();
        }
				
        public int getSize() {
            return (SIZE_OF_BYTE + SIZE_OF_INT  + 3);
        }
			
        public String toDebugString() {
            return String.format("mFlag:%x, mDls:%d", mFlag, mDls);
        }
    };   
    /* Gionee liuxw 2013-07-13 add for gn_correlative_nv end */
}

...
static sns_err_code_e sns_fill_debug_str_mask(sns_debug_mask_t* debug_str_mask)
{
  ...
  if ((dbg_config_file_ptr = fopen(SNS_DBG_CONFIG_FILE_NAME,"r")) == NULL)
  {
    SNS_PRINTF_STRING_ERROR_0(DBG_MOD_INIT,
                              "Debug Config File missing in EFS!");
/*Gionee luliya 2013-11-16 modify for logcat failure after Restore  factory settings begin */
   

   module_mask_arr[SNS_DBG_MOD_APPS_INIT] = 1;
   module_mask_arr[SNS_DBG_MOD_DSPS_SMGR] = 1;
   module_mask_arr[SNS_DBG_MOD_DSPS_SAM] = 1;

   module_mask_arr[SNS_DBG_MOD_DSPS_DD_ACCEL] = 1;
   module_mask_arr[SNS_DBG_MOD_DSPS_DD_GYRO] = 1;
   module_mask_arr[SNS_DBG_MOD_DSPS_DD_ALSPRX] = 1;
   module_mask_arr[SNS_DBG_MOD_DSPS_DD_MAG8975] = 1;
    //return SNS_ERR_FAILED;
...
/*Gionee luliya 2013-11-16 modify for logcat failure after Restore  factory settings end */
  /* Construct the bit mask from the array */
  *debug_str_mask = 0;
  for(i=0; i < SNS_DBG_NUM_MOD_IDS; i++)
  {
